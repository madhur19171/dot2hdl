...
//This version of dot parser has been modified to support NoC
...
void printNode(NODE_T node){
	...
	for(int i = 0; i < node.outputs.size; i++){
		cout << "\n\n#" << (i + 1) << "\nBit Size: " << node.outputs.output[i].bit_size
				...
				<< "\tNext Nodes Port: " << node.outputs.output[i].next_nodes_port
...
int get_component_node_id ( string parameters )
{
	parameters = string_clean( parameters );
...
IN_T get_component_inputs ( string in , int components_in_netlist )
{
	...
	if(par[1].find("in0") != std::string::npos)//To support Router indexing from 0
		inputs.size++;
...
OUT_T get_component_outputs ( string parameters )
{
	...
	if(par[1].find("out0") != std::string::npos)//To support Router indexing from 0
		outputs.size++;
...
void parse_connections ( string line )
{
	...
	if ( v.size() > 0 )
	{
		...
		for (i = 0; i < components_in_netlist; i++)
		{
			...
			{
				current_node_id = i;//Index of node in NODES array
				break;
			}
		}
		...
		bool Router = false;//If this is false, then the InputOutput indexing will be normal
		//But if it is true, input output indexing will start from 0, so output_indx-- and intput_indx-- will not be required

		for ( indx = 0; indx < parameters.size(); indx++ )
		{
			if(parameters[indx].find("Router") != std::string::npos)
				Router = true;
		}
		...
		for ( indx = 0; indx < parameters.size(); indx++ )
		{
			if ( parameters[indx].find("from") != std::string::npos )
			{
				...
				if(!Router)//Needs to be changed for the case when Router and basic components are connected in the same file
					output_indx--;
			}
